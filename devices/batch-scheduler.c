...
//			printf("IN LOW packets in bus:  %d\n", packets_in_bus.value);	
			...
			/*
			if(task.direction == SENDER){
			cond_wait (&get_on_low_send, &blocked);
			}else{
			cond_wait (&get_on_low_rec, &blocked);
			}*/
		}
...
/*	
	if(task.direction == SENDER){
		if(task_send_prio.value >0)	
			cond_signal (&get_on_high_send, &blocked);
		else if(task_receive_prio.value > 0 ||bussSlotsFree==3)
			cond_signal (&get_on_high_rec, &blocked);
		else if(task_send_prio.value==0 && task_receive_prio.value==0){
			if(task_send_low.value>0)  	
				cond_signal (&get_on_low_send, &blocked);
			else
				cond_signal (&get_on_low_rec, &blocked);
		}	
